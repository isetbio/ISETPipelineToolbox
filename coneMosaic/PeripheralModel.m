classdef PeripheralModel
    ...
    methods(Static)
        ...
        function [mosaic, psfObj, psfData, zCoeffs] = eyeModelCmosaic(eccX, eccY, fovDegs, pupilDiam, randomMesh, subjectID, varargin)
            ...
            % We have to have the mosaic object to do this.
            [oiEnsemble, psfEnsemble, zCoeffs] = mosaicTemp.oiEnsembleGenerate(mosaicEcc, ...
            ...
            clear psfObj psfData oiEnsemble zCoeffs
            
            % Regenerate mosaic centered at target eccentricity, using
            % microns per degree from the oi.
            mosaic = cMosaic(...
                'sizeDegs', [1 1] * fovDegs, ...
                'computeMeshFromScratch', randomMesh, ...
                'randomSeed', randomSeed, ...       % set the random seed to generate a different mosaic each time, or not
            ...
            % Regenerate optics from the actual mosaic we're using
            % We have to have the mosaic object to do this.
